...
fn bench_gvas_read(c: &mut Criterion) {
    ...
    let mut gvas: GvasFile =
        GvasFile::read(&mut Cursor::new(&bytes), GameVersion::Default).unwrap();
    group.bench_function("get a deep struct (time)", |b| {
        b.iter(|| {
            let g = black_box(&gvas);

            let res = PokemonInfo::new_party(g).unwrap().get_stats(2).unwrap();
    ...
    pub static FAKE_STATIC: OnceLock<Arc<RwLock<GvasFile>>> =
        OnceLock::<Arc<RwLock<GvasFile>>>::new();
