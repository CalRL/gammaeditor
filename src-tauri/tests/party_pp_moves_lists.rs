...
fn moves_at_returns_moves() {
    ...
    let moves: Option<&ArrayProperty> = PartyPPMovesLists::moves_at(array, 1);
...
fn max_pp_at_returns() {
    let gvas: GvasFile = get_gvas();
    let prop: &Property = gvas.properties.get("PartyPPMovesLists").expect("get prop");
    let moves = PartyPPMovesLists::moves_array(prop).expect("get moves");
    let moves_at = PartyPPMovesLists::moves_at(moves, 0).expect("get moves at");
    let max_pp = PartyPPMovesLists::max_pp_at(moves_at, 0).expect("hi").clone();
...
fn current_pp_returns() {
    ...
    let moves = PartyPPMovesLists::moves_array(prop).expect("get moves");
    let moves_at = PartyPPMovesLists::moves_at(moves, 0).expect("get moves at");
    let current_pp = PartyPPMovesLists::current_pp_at(moves_at, 1).expect("hi").clone();
