...
pub fn moves_at(array: &ArrayProperty, index: usize) -> Option<&ArrayProperty> {
    let structs: &Vec<StructProperty> = match array {
        ArrayProperty::Structs { structs, .. } => structs,
        _ => return None,
    };

    let sp: &StructProperty = structs.get(index)?;
    let properties: &IndexMap<String, Vec<Property>> = match &sp.value {
...
pub fn max_pp_at(moves: &ArrayProperty, index: usize) -> Option<&i32> {
    let pp_struct: &StructProperty = match &moves {
        ArrayProperty::Structs { structs, .. } => structs.get(index)?,
        _ => return None
    };
    let map: &IndexMap<String, Vec<Property>> = pp_struct.value.get_custom_struct()?.1;
    let prop: &Property = map.iter()
